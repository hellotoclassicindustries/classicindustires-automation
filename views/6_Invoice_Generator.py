...
# ============================================================================
# 2. REPORTLAB AUTOMATED MULTI-ITEM EXECUTIVE PDF GENERATION ENGINE
# ============================================================================
def generate_invoice_pdf_file(data):
    ...
    # Initialize basic layout template document geometries (Print-Safe margins)
    ...
    # Metadata Chronological Dual-Column Construction (Total width = 540)
    meta_data = [
    ...
    meta_table = Table(meta_data, colWidths=[270, 240])
    ...
    # Apply calibrated widths and high-contrast styles to table (Total width = 540)
    billing_table = Table(table_content, colWidths=[160, 55, 65, 65, 85, 110])
    t_style = [
    ...
    return pdf_filename
# ============================================================================
# 3. DATE RANGE & DROPDOWN FILTER INPUT MATRIX
# ============================================================================
...
# 📆 SIMPLIFIED SELECTION GROUP: Clear calendar & filtering rules side-by-side
...
if not is_filtered_run:
    ...
    # 🚀 FIXED SCALAR INDEXING: Safe conversion of row item slice series to unique scalar dictionaries
...
# ============================================================================
# 4. COMPILER TRIGGER AND DOCUMENT DISTRIBUTION GATEWAY
# ============================================================================
st.markdown("---")
...
with col_d1:
    generated_serial = f"CI/{start_date.strftime('%Y')}-{end_date.strftime('%y')}/{datetime.now().strftime('%M%S')}"
    invoice_no = st.text_input("Invoice Serial Code", value=generated_serial)
...
if st.button("🚀 Compile Print-Ready Executive Multi-Item Invoice PDF", use_container_width=True):
    with st.spinner("Generating cryptographically sound multi-item document structure..."):
        try:
            # Executes cleanly with perfectly calibrated table column widths
