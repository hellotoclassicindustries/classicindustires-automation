...
# ============================================================================
# PART 1: INITIALIZATION & STATE MANAGEMENT CORE
# ============================================================================
...
# ============================================================================
# PART 4: PRINT PREVIEW & DATA ACCUMULATION ENGINE
# ============================================================================
st.markdown("---")
...
for idx, row in merged_summary.iterrows():
    ...
    if not match_part.empty:
        if "weight_kg" in match_part.columns:
            ...
            if pd.notna(val_wt): p_weight_kg = float(val_wt)
                
        if "rate_per_ton" in match_part.columns:
            val_rt = match_part["rate_per_ton"].iat[0]
            if pd.notna(val_rt): p_rate = float(val_rt)
        
        hsn_val = None
        if "hsn_code" in match_part.columns: hsn_val = match_part["hsn_code"].iat[0]
        elif "hsn_sac" in match_part.columns: hsn_val = match_part["hsn_sac"].iat[0]
...
with col_btn2:
    if st.button("🔓 Modify Parameters / Unfreeze Form", width="stretch", disabled=not st.session_state.invoice_staged):
        ...
        st.rerun()
# ============================================================================
# PART 5: PDF BLOCKS MATRIX GENERATOR
# ============================================================================
